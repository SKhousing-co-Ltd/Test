...
「外部連携受信箱」には、未登録の取引先名を正規化して集約した候補一覧を表示します。管理者・マネージャーは、発注明細数と発注額合計を確認し、登録する候補だけを選択して「登録・再照合」を実行します。

- 日次同期だけでは取引先マスタを自動作成しません。
- 選択した候補だけを取引先マスタへ登録し、同じ名称のAppSuite発注とBill One請求書を再照合します。
- 既存の取引先が1件なら再利用し、無効化済みなら有効化します。
- 同じ正規化名の既存取引先が複数ある場合は自動処理せず、候補を整理してから再実行します。
- RPCは `security invoker` で動作し、実行できるのは有効な管理者・マネージャーだけです。匿名利用者には公開しません。
