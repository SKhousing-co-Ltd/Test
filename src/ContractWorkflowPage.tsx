...
export function ContractWorkflowPage({ canComplete }: { canComplete: boolean }) {
  ...
  const beginLink = async (item: WorkflowItem) => {
    if (!supabase) return;
    setError(''); setNotice(''); setLinkingItem(item); setCandidates([]); setSelectedContractId(''); setSelectedUnitIds([]);
  ...
  const linkAndComplete = async () => {
    ...
    setLinkingItem(null); setNotice('契約を紐づけ、ワークフローを処理済みにしました。'); await load();
  };
