...
type GeoJSONMultiPolygon = { type: 'MultiPolygon'; coordinates: Point[][][] };
...
function toMultiPolygon(points: Point[]): GeoJSONMultiPolygon { const last = points[points.length - 1]; const ring = points.length && (points[0][0] !== last?.[0] || points[0][1] !== last?.[1]) ? [...points, points[0]] : points; return { type: 'MultiPolygon', coordinates: [[ring]] }; }
function pathFor(geometry: GeoJSONMultiPolygon) { return geometry.coordinates.flatMap((polygon) => polygon.map((ring) => `M ${ring.map(([x, y]) => `${x} ${y}`).join(' L ')} Z`)).join(' '); }
function centerFor(geometry: GeoJSONMultiPolygon): Point { const points = geometry.coordinates[0]?.[0] ?? [[.5, .5]]; const unique = points.slice(0, -1); return [unique.reduce((sum, point) => sum + point[0], 0) / unique.length, unique.reduce((sum, point) => sum + point[1], 0) / unique.length]; }
function fileKind(file: File): 'png' | 'jpeg' | 'svg' | 'pdf' | null { if (file.type === 'application/pdf') return 'pdf'; if (file.type === 'image/svg+xml') return 'svg'; if (file.type === 'image/png') return 'png'; if (file.type === 'image/jpeg') return 'jpeg'; return null; }
function scopeKey(plan: Pick<FloorPlanSummary, 'propertyId' | 'floorLabel'>) { return `${plan.propertyId}|${plan.floorLabel}`; }
...
export function LeasingMapPage() {
  ...
  const updateStatus = async (status: Exclude<DisplayStatus, 'occupied'>) => { if (!supabase || !selectedUnitId) return; const { error } = await supabase.rpc('set_unit_leasing_status', { p_unit_id: selectedUnitId, p_status: status }); if (error) return setNotice(error.message); setNotice('リーシング状態を更新しました。'); await loadRevisionData(selectedRevisionId); };
