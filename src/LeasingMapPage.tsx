...
type Unit = { unit_id: string; unit_code: string; unit_name: string | null; rentable_area_sqm: number | null; contract_units: Array<{ lease_contract_unit_id: string }> | null };
...
const compareLabels = (left: string, right: string) => left.localeCompare(right, 'ja-JP', { numeric: true, sensitivity: 'base' });
...
export function LeasingMapPage() {
  ...
  const selectedPropertyId = selectedPlan?.propertyId ?? '';
  const propertyOptions = [...new Map(plans.map((plan) => [plan.propertyId, plan.propertyName])).entries()].map(([id, name]) => ({ id, name })).sort((left, right) => compareLabels(right.name, left.name));
  const floorOptions = plans.filter((plan) => plan.propertyId === selectedPropertyId).sort((left, right) => compareLabels(right.floorLabel, left.floorLabel));
  ...
  const loadRevisionData = async (revisionId: string) => {
    ...
    if (plan) { const { data } = await supabase.from('unit_master').select('unit_id, unit_code, unit_name, rentable_area_sqm, contract_units:lease_contract_unit(lease_contract_unit_id)').eq('property_id', plan.propertyId).eq('floor_label', plan.floorLabel).eq('is_active', true).order('unit_code'); setUnits((data ?? []) as Unit[]); }
  };
  ...
  const deleteUnit = async (unit: Unit) => {
    if (!supabase || !selectedRevisionId || unit.contract_units?.length) return;
    if (!window.confirm(`「${unit.unit_code}${unit.unit_name ? ` ${unit.unit_name}` : ''}」を削除します。図面上の区画形状とリーシング状態も削除され、この操作は取り消せません。`)) return;
    setDeletingUnitId(unit.unit_id);
    const { error } = await supabase.rpc('delete_map_unit', { p_unit_id: unit.unit_id });
    setDeletingUnitId(null);
    if (error) return setNotice(`区画を削除できません: ${error.message}`);
    if (selectedUnitId === unit.unit_id) setSelectedUnitId('');
    setNotice(`区画「${unit.unit_code}」を削除しました。`);
    await loadPlans();
    await loadRevisionData(selectedRevisionId);
  };
  ...
  return <section className="leasing-page">
    ...
    {!plans.length ? <div className="empty-state"><strong>登録済みのフロアがありません</strong><p>先に既存の区画マスタへ物件とフロアを登録してください。</p></div> : <>
      <div className="leasing-toolbar"><div className="leasing-scope-selectors"><label>物件<select value={selectedPropertyId} onChange={(event) => { const nextPlan = plans.find((plan) => plan.propertyId === event.target.value); if (nextPlan) setSelectedPlanId(scopeKey(nextPlan)); }}>{propertyOptions.map((property) => <option value={property.id} key={property.id}>{property.name}</option>)}</select></label><label>フロア<select value={selectedPlanId} onChange={(event) => setSelectedPlanId(event.target.value)}>{floorOptions.map((plan) => <option value={scopeKey(plan)} key={scopeKey(plan)}>{plan.floorLabel}</option>)}</select></label></div><label>図面版<select value={selectedRevisionId} onChange={(event) => setSelectedRevisionId(event.target.value)}>{revisions.map((revision) => <option value={revision.id} key={revision.id}>v{revision.revisionNo}（{new Date(revision.createdAt).toLocaleDateString('ja-JP')}）</option>)}</select></label><span className="revision-meta">{selectedRevision?.fileType.toUpperCase()}{selectedRevision?.pdfPage ? ` / ${selectedRevision.pdfPage}ページ` : ''}</span></div>
      {selectedRevisionId ? <div className="leasing-workspace"><div className="map-panel"><div className="map-tools"><button className={tool === 'select' ? 'selected' : ''} onClick={() => { setTool('select'); setDraft([]); }}>選択</button><button className={tool === 'polygon' ? 'selected' : ''} onClick={() => { setTool('polygon'); setDraft([]); }}>多角形</button><button className={tool === 'rectangle' ? 'selected' : ''} onClick={() => setTool('rectangle')}>矩形</button>{tool === 'polygon' && <button className="map-save" onClick={() => void finishPolygon()}>多角形を確定</button>}<div className="zoom-controls" aria-label="表示倍率"><button onClick={() => setZoom((value) => Math.max(50, value - 25))} disabled={zoom <= 50} aria-label="縮小">−</button><button className="zoom-value" onClick={() => setZoom(100)} title="100%に戻す">{zoom}%</button><button onClick={() => setZoom((value) => Math.min(200, value + 25))} disabled={zoom >= 200} aria-label="拡大">＋</button></div><span>{selectedUnitId ? `編集対象: ${units.find((unit) => unit.unit_id === selectedUnitId)?.unit_code ?? ''}` : '編集対象の区画を選択'}</span></div><div className="map-stage"><div className="map-canvas" style={{ width: `${zoom}%` }}><img src={previewUrl} alt="フロア図面" /><svg ref={stageRef} viewBox="0 0 1 1" preserveAspectRatio="none" onPointerDown={onStagePointerDown} onPointerMove={onStagePointerMove} onPointerUp={() => void onStagePointerUp()}>{features.map((feature) => { const [x, y] = centerFor(feature.geometry); return <g key={feature.unitId} className={selectedUnitId === feature.unitId ? 'map-feature selected' : 'map-feature'} onClick={(event) => { event.stopPropagation(); setSelectedUnitId(feature.unitId); }}><path d={pathFor(feature.geometry)} style={{ fill: statusColor[feature.displayStatus], stroke: statusColor[feature.displayStatus] }} /><text x={x} y={y}>{feature.unitCode}</text></g>; })}{draft.length > 0 && <path className="draft-shape" d={pathFor(toMultiPolygon(draft))} />}{rectStart && rectEnd && <path className="draft-shape" d={pathFor(toMultiPolygon([[rectStart[0], rectStart[1]], [rectEnd[0], rectStart[1]], rectEnd, [rectStart[0], rectEnd[1]]]))} />}</svg></div></div><div className="map-legend">{(Object.keys(statusLabel) as DisplayStatus[]).map((status) => <span key={status}><i style={{ background: statusColor[status] }} />{statusLabel[status]}</span>)}</div></div>
        <aside className="map-sidebar"><section><div className="section-title-row"><h3>区画</h3><button className="add-unit-button" onClick={() => setUnitCreateOpen(true)}>＋ 区画を追加</button></div><p className="unit-list-hint">候補は契約情報ではなく、この物件・フロアの区画マスタです。契約に紐付く区画は削除できません。</p><div className="unit-list">{units.map((unit) => { const feature = features.find((item) => item.unitId === unit.unit_id); const isContractLinked = Boolean(unit.contract_units?.length); return <div className="unit-list-row" key={unit.unit_id}><button className={selectedUnitId === unit.unit_id ? 'selected' : ''} onClick={() => setSelectedUnitId(unit.unit_id)}><strong>{unit.unit_code}</strong><span>{unit.rentable_area_sqm ? `${unit.rentable_area_sqm}㎡` : '面積未設定'} {feature ? statusLabel[feature.displayStatus] : '図形未登録'}{isContractLinked ? ' / 契約紐付あり' : ''}</span></button><button className="delete-unit-button" onClick={() => void deleteUnit(unit)} disabled={isContractLinked || deletingUnitId === unit.unit_id} title={isContractLinked ? '契約に紐付いている区画は削除できません' : 'この区画を削除'}>{deletingUnitId === unit.unit_id ? '削除中…' : '削除'}</button></div>; })}</div></section>{selectedFeature && <section className="status-editor"><h3>{selectedFeature.unitCode} の状態</h3><div>{(['occupied', 'vacant', 'applied', 'unavailable'] as const).map((status) => <button onClick={() => void updateStatus(status)} className={selectedFeature.displayStatus === status ? 'selected' : ''} key={status}>{statusLabel[status]}</button>)}</div>{selectedFeature.displayStatus === 'occupied' && <p>有効な契約がある場合も「契約中」と表示されます。</p>}</section>}<section><h3>変更履歴</h3><ol className="history-list">{history.map((change) => <li key={change.id}><strong>{change.type}</strong><span>{new Date(change.createdAt).toLocaleString('ja-JP')}</span></li>)}</ol></section></aside></div> : <div className="empty-state"><strong>図面版がありません</strong><p>このフロアのベース図面を登録してください。</p></div>}
    </>}
