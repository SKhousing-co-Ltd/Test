...
export function PropertyBillingSettings({ propertyId, properties, canEdit }: { propertyId: string; properties: BillingProperty[]; canEdit: boolean }) {
  const [activeTab, setActiveTab] = useState<'types' | 'items' | 'patterns'>('types');
  ...
  return <section className="property-billing-settings"><header><div><h3>請求設定</h3><p>{property ? `${property.short_name || property.asset_name}の明細項目と請求期間パターンを設定します。` : '物件を選択してください。'}</p></div></header>{!databaseReady && !loading && <p className="property-billing-settings-notice">現在はローカル表示用の仮設定です。データベース移行後は物件ごとの設定として保存されます。</p>}{notice && <p className="property-billing-settings-notice error">{notice}</p>}
    <div className="property-billing-tabs"><button type="button" className={activeTab === 'types' ? 'active' : ''} onClick={() => setActiveTab('types')}>請求種別</button><button type="button" className={activeTab === 'items' ? 'active' : ''} onClick={() => setActiveTab('items')}>明細項目</button><button type="button" className={activeTab === 'patterns' ? 'active' : ''} onClick={() => setActiveTab('patterns')}>請求期間の任意パターン</button></div>
    {activeTab === 'types' ? <PropertyBillingChargeTypeSettings propertyId={propertyId} canEdit={canEdit} /> : activeTab === 'items' ? <section className="property-billing-items"><div className="property-billing-items-heading"><h4>明細項目</h4><p>この物件で請求する項目を登録します。固定費の請求期間は契約情報から取得します。</p></div><form onSubmit={addItem} className="property-billing-item-form"><label>項目<input value={lineItemName} onChange={(event) => setLineItemName(event.target.value)} placeholder="例：電気代（基本料）" /></label><label>明細表示名<input value={displayName} onChange={(event) => setDisplayName(event.target.value)} placeholder="例：電気代" /></label><label>請求種別<select value={chargeTypeId} onChange={(event) => setChargeTypeId(event.target.value)}>{chargeTypes.map((type) => <option key={type.billing_charge_type_id} value={type.billing_charge_type_id}>{type.charge_type_name}</option>)}</select></label><label>請求区分<select value={billingKind} onChange={(event) => setBillingKind(event.target.value as 'fixed' | 'variable')}><option value="fixed">固定費</option><option value="variable">変動費</option></select></label><label>請求期間<select disabled={billingKind === 'fixed'} value={billingKind === 'fixed' ? 'contract' : periodRule} onChange={(event) => setPeriodRule(event.target.value as PeriodRule)}><option value="contract">契約情報から設定</option><option value="manual">手入力</option><option value="meter_reading">検針データから設定</option><option value="custom_pattern">任意のパターンから設定</option></select></label><button type="submit" className="primary-button" disabled={!canEdit || !chargeTypes.length}>登録</button></form></section> : <section className="property-period-patterns"><div><h4>請求期間の任意パターン</h4><p>任意パターン用に、請求書へ表示する期間を物件ごとに登録します。</p></div><form onSubmit={addPattern}><label>パターン名<input value={patternName} onChange={(event) => setPatternName(event.target.value)} placeholder="例：前月分" /></label><button type="submit" className="primary-button" disabled={!canEdit}>登録</button></form></section>}
  </section>;
}
