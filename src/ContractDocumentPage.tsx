...
import { ContractTemplateLayoutEditor } from './ContractTemplateLayoutEditor';
...
type StoredDocument = { lease_contract_document_id: string; document_type: DocumentType; contract_document_template_revision_id: string | null; field_values: Values; workflow_defaults: Values; manually_edited_fields: string[]; desknets_application_id: string | null; pdf_file_path: string | null; pdf_generated_at: string | null };
...
export function ContractDocumentPage() {
  ...
  const load = async () => {
    ...
    const [{ data: contract, error: contractError }, { data: stored, error: documentError }] = await Promise.all([
      supabase.from('lease_contract').select('lease_contract_id, contract_start_date, contract_end_date, payment_terms, notes, tenant:tenant_master(tenant_name), contract_units:lease_contract_unit(leased_area_sqm, monthly_rent_amount, monthly_common_charge_amount, deposit_amount, security_deposit_amount, key_money_amount, unit:unit_master(unit_id, unit_code, property_id, floor_label, rentable_area_sqm, property:asset_master(property_name:asset_name, address, building_area_sqm)))').eq('lease_contract_id', contractId).maybeSingle(),
      supabase.from('lease_contract_document').select('lease_contract_document_id, document_type, contract_document_template_revision_id, field_values, workflow_defaults, manually_edited_fields, desknets_application_id, pdf_file_path, pdf_generated_at').eq('lease_contract_id', contractId).maybeSingle(),
    ]);
    ...
    setValues(existing ? { ...rentalDefaults, ...existing.field_values } : rentalDefaults); setWorkflowDefaults(existing?.workflow_defaults ?? emptyValues()); setManualFields(existing?.manually_edited_fields ?? []); setDocument(existing); setDocumentType(existing?.document_type ?? 'ordinary_lease');
  ...
  const save = async (nextValues = values, nextDefaults = workflowDefaults, nextManualFields = manualFields, application?: WorkflowApplication): Promise<StoredDocument | null> => {
    ...
    const payload: Record<string, unknown> = { lease_contract_id: contractId, document_type: documentType, contract_document_template_revision_id: document?.contract_document_template_revision_id, field_values: nextValues, workflow_defaults: nextDefaults, manually_edited_fields: nextManualFields, desknets_application_id: application?.applicationId ?? document?.desknets_application_id ?? null };
    if (application) { payload.desknets_retrieved_at = new Date().toISOString(); payload.desknets_source_payload = application.source; }
    const { data, error: saveError } = await supabase.from('lease_contract_document').upsert(payload, { onConflict: 'lease_contract_id' }).select('lease_contract_document_id, document_type, contract_document_template_revision_id, field_values, workflow_defaults, manually_edited_fields, desknets_application_id, pdf_file_path, pdf_generated_at').single();
  ...
  const generate = async () => { if (!supabase) return; if (isDemo) { await refreshPreview(); setNotice('右側に新ひな型PDFを表示しました。デモでは平面図欄は空欄です。'); return; } const needsPlan = ['ordinary_lease', 'fixed_term_building_lease', 'warehouse'].includes(documentType); if (needsPlan && !selectedUnitIds.length) { setError('PDFを確認する前に、平面図へ転記する区画を選択してください。'); return; } if (!await saveAll()) return; setSaving(true); setError(''); const { data, error: functionError } = await supabase.functions.invoke('generate-contract-pdf', { body: { leaseContractId: contractId, documentType } }); setSaving(false); if (functionError || data?.error) { setError(data?.error ?? `PDFを生成できませんでした: ${functionError?.message ?? ''}`); return; } setNotice('PDFを生成して右側プレビューへ反映しました。'); await refreshPreview(data.pdfFilePath); await load(); };
  ...
  return <section className="contract-document-page"><header className="document-heading"><div><p className="section-kicker">{isDemo ? 'DEMO / ORDINARY LEASE' : 'CONTRACT DOCUMENT'}</p><h2>貸室賃貸借契約書</h2><p>指定ひな型へ契約情報と対象区画図を転記してPDFを生成します。</p></div><div><Link className="secondary-button" to="/contracts">契約一覧へ戻る</Link><button className="primary-button" onClick={() => void generate()} disabled={saving}>PDFを確認</button></div></header><ContractTemplateTabs contractId={contractId} currentType={documentType} readOnly={isDemo} onChanged={() => void load()} /><ContractTemplateLayoutEditor documentType={documentType} />
