...
export function PropertyBillingChargeTypeSettings({ propertyId, canEdit, onChange }: { propertyId: string; canEdit: boolean; onChange?: (ids: string[]) => void }) {
  ...
  useEffect(() => { const load = async () => { if (!supabase || !propertyId) { setTypes([]); setEnabled([]); return; } const [typesResult, settingResult] = await Promise.all([supabase.from('billing_charge_type').select('billing_charge_type_id, charge_type_name').eq('is_active', true).order('sort_order'), supabase.from('asset_billing_charge_type_setting').select('billing_charge_type_id').eq('asset_id', propertyId).eq('is_enabled', true)]); if (!typesResult.error && !settingResult.error) { setTypes((typesResult.data ?? []) as ChargeType[]); setEnabled((settingResult.data ?? []).map((row) => row.billing_charge_type_id)); setDatabaseReady(true); return; } setTypes([]); setEnabled([]); setDatabaseReady(false); }; void load(); }, [propertyId]);
  const toggle = async (id: string, checked: boolean) => { if (!databaseReady || !supabase) return; const next = checked ? [...enabled, id] : enabled.filter((item) => item !== id); setEnabled(next); onChange?.(next); await supabase.from('asset_billing_charge_type_setting').upsert({ asset_id: propertyId, billing_charge_type_id: id, is_enabled: checked }, { onConflict: 'asset_id,billing_charge_type_id' }); };
