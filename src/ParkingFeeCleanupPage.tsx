...
export function ParkingFeeCleanupPage({ role }: { role: Role }) {
  ...
  const applyBulkEndDate = () => {
    ...
    setEndDates((current) => ({ ...current, ...Object.fromEntries(selectedRows.map((row) => [row.requestId, bulkEndDate])) })); setError('');
  };
  const confirm = async () => {
