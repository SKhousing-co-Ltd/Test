...
export function InvoiceCreationPage({ propertyId, propertyName, period }: { propertyId: string; propertyName: string; period: BillingPeriod }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      for (const [invoiceKey, invoice] of invoices) {
        ...
        [...invoice.lines.entries()].forEach(([name, amount], index) => { const values = emptyValues(); if (index === 0) { values[0] = String(invoiceNumber); values[1] = invoice.code; values[2] = invoice.displayName || invoice.tenantName; values[3] = invoice.subject || `${propertyName}${[...invoice.unitNames].join('・')} 御請求書`; values[4] = dueDate(calendarYear, period.month, nextDuePatterns[0]); values[5] = yen.format(total); values[6] = yen.format(tax); values[7] = yen.format(total + tax); }
          values[10] = '未設定'; values[11] = name; values[12] = '1'; values[13] = '式'; values[14] = ''; values[15] = yen.format(amount); values[18] = '課税'; values[19] = '10%（仮）'; next.push({ id: `${invoiceKey}:${index}:${name}`, invoiceKey, values }); });
      }
      setRows(next);
  ...
  return <section className="invoice-creation-page"><header className="invoice-creation-heading"><div><p className="section-kicker">INVOICE CSV</p><h3>請求書作成</h3><p>{propertyName}・{calendarYear}年{period.month}月分</p></div><div className="invoice-creation-actions"><label><input type="checkbox" checked={showInvoiceNote} onChange={(event) => setInvoiceNoteEnabled(event.target.checked)} />請求書備考を表示</label><input value={invoiceNote} disabled={!showInvoiceNote} onChange={(event) => updateInvoiceNote(event.target.value)} placeholder="全請求書共通の備考" /><button className="primary-button" disabled>CSVを出力</button></div></header>
    ...
    <div className="invoice-creation-table-wrap invoice-csv-table-wrap"><table className="invoice-csv-table"><thead><tr className="invoice-total-row">{visibleColumns.map((column) => <th key={`total-${csvHeaders[column]}`}>{totalColumns.has(column) ? <><span>表示中合計</span><strong>{yen.format(totals[column as keyof typeof totals])}</strong></> : null}</th>)}</tr><tr>{visibleColumns.map((column) => <th key={csvHeaders[column]}>{column === 10 || column === 11 ? <label>{screenHeaders[column]}<select value={column === 10 ? lineItem1Filter : lineItem2Filter} onChange={(event) => column === 10 ? setLineItem1Filter(event.target.value) : setLineItem2Filter(event.target.value)}><option value="all">すべて</option>{(column === 10 ? lineItem1Options : lineItem2Options).map((option) => <option key={option} value={option}>{option}</option>)}</select></label> : screenHeaders[column]}</th>)}</tr></thead><tbody>{loading ? <tr><td colSpan={visibleColumns.length} className="invoice-creation-empty">レントロールを読み込み中…</td></tr> : !filteredRows.length ? <tr><td colSpan={visibleColumns.length} className="invoice-creation-empty">条件に一致する固定費の契約データがありません。</td></tr> : filteredRows.map((row) => { const firstVisible = firstVisibleRowIds.has(row.id); const summary = invoiceSummaryByKey.get(row.invoiceKey) ?? row; return <tr key={row.id} className={firstVisible ? 'invoice-start-row' : 'invoice-continuation-row'}>{visibleColumns.map((column) => <td key={`${row.id}-${column}`}>{column === 4 && firstVisible ? <select className="invoice-due-pattern" aria-label={`入金期日パターン ${row.invoiceKey}`} value={duePatternByInvoice[row.invoiceKey] ?? ''} onChange={(event) => selectDuePattern(row.invoiceKey, event.target.value)}><option value="">未選択</option>{duePatterns.map((pattern) => <option key={pattern.billing_due_date_pattern_id} value={pattern.billing_due_date_pattern_id}>{duePatternLabel(pattern)}</option>)}</select> : invoiceLevelColumns.has(column) && !firstVisible ? null : <input aria-label={`${screenHeaders[column]} ${row.id}`} value={(invoiceLevelColumns.has(column) ? summary : row).values[column]} readOnly={column === 0} onChange={(event) => invoiceLevelColumns.has(column) ? updateInvoiceCell(row.invoiceKey, column, event.target.value) : updateCell(row.id, column, event.target.value)} />}</td>)}</tr>; })}</tbody></table></div>
  </section>;
}
