...
export function InvoiceCreationPage({ propertyId, propertyName, period }: { propertyId: string; propertyName: string; period: BillingPeriod }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setPeriodPatternByRow({}); setPeriodTextByPattern(Object.fromEntries(nextPeriodPatterns.map((pattern) => [pattern.billing_period_pattern_id, periodText(calendarYear, period.month, pattern)])));
  ...
  const selectPeriodPattern = (rowId: string, patternId: string) => { setPeriodPatternByRow((current) => ({ ...current, [rowId]: patternId })); setLineItem1Filter('all'); updateCell(rowId, 10, patternId ? (periodTextByPattern[patternId] || '') : '未設定'); };
  // 上部の請求期間を書き換えると、そのパターンを選んでいる明細すべてに反映します。
  const changePeriodText = (patternId: string, value: string) => { setPeriodTextByPattern((current) => ({ ...current, [patternId]: value })); setLineItem1Filter('all'); setRows((current) => current.map((row) => (periodPatternByRow[row.id] ?? '') === patternId ? { ...row, values: row.values.map((cell, index) => index === 10 ? value : cell) } : row)); };
  ...
  return <section className="invoice-creation-page"><header className="invoice-creation-heading"><div><p className="section-kicker">INVOICE CSV</p><h3>請求書作成</h3><p>{propertyName}・{calendarYear}年{period.month}月分</p></div><div className="invoice-creation-actions"><label><input type="checkbox" checked={showInvoiceNote} onChange={(event) => setInvoiceNoteEnabled(event.target.checked)} />請求書備考を表示</label><input value={invoiceNote} disabled={!showInvoiceNote} onChange={(event) => updateInvoiceNote(event.target.value)} placeholder="全請求書共通の備考" /><button className="primary-button" disabled>CSVを出力</button></div></header>
    <div className="invoice-date-panel">
      <section><h4>入金期限</h4>{duePatterns.length ? <ul>{duePatterns.map((pattern) => <li key={pattern.billing_due_date_pattern_id}><span title={duePatternLabel(pattern)}>{duePatternMark(pattern)}</span><input value={dueDateByPattern[pattern.billing_due_date_pattern_id] ?? ''} placeholder="YYYY/M/D" aria-label={`入金期限 ${duePatternLabel(pattern)}`} onChange={(event) => changeDueDate(pattern.billing_due_date_pattern_id, event.target.value)} /></li>)}</ul> : <p>入金期日パターンは請求設定で登録してください。</p>}</section>
      <section><h4>請求期間</h4>{periodPatterns.length ? <ul>{periodPatterns.map((pattern) => <li key={pattern.billing_period_pattern_id}><span>{pattern.pattern_name}</span><input value={periodTextByPattern[pattern.billing_period_pattern_id] ?? ''} placeholder="YYYY/M/D～YYYY/M/D" aria-label={`請求期間 ${pattern.pattern_name}`} onChange={(event) => changePeriodText(pattern.billing_period_pattern_id, event.target.value)} /></li>)}</ul> : <p>請求期間パターンは請求設定で登録してください。</p>}</section>
    </div>
