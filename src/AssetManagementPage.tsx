...
export function AssetManagementPage() {
  ...
  return <>
    <section className="page-heading"><div><p className="section-kicker">SETTINGS</p><h2>アセット設定</h2><p>アセットごとのレントロール表示とテナント請求対象を設定します。</p></div><button className="primary-button" disabled={saving || !changedAssets.length} onClick={() => void save()}>{saving ? '保存中…' : `${changedAssets.length ? `${changedAssets.length}件を` : ''}保存`}</button></section>
    <section className="asset-management-notes"><p><strong>レントロール表示</strong>を有効にしたアセットだけがレントロール画面に表示されます。</p><p><strong>テナント請求</strong>を有効にしたアセットだけがテナント請求・今後の請求処理の対象になります。</p></section>
