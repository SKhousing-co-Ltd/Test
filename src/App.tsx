...
import { MeterAllocationPrototypePage } from './MeterAllocationPrototypePage';
      ...
      <Routes>
        ...
        <Route element={<ProtectedRoute session={session} profile={profile} isLoading={isLoading} />}>
          <Route element={<PortalLayout profile={profile!} onSignOut={signOut} />}>
            ...
            <Route path="/admin/meter-allocation-prototype" element={<AdminOnly role={profile?.role ?? 'viewer'}><MeterAllocationPrototypePage /></AdminOnly>} />
...
function PortalLayout({ profile, onSignOut }: { profile: UserProfile; onSignOut: () => Promise<void> }) {
  ...
  const pageTitle = location.pathname === '/appsuite-sync' ? 'AppSuite同期' : location.pathname === '/change-requests' ? '対応依頼' : location.pathname === '/admin/parking-fee-cleanup' ? '駐車料金一括整備' : location.pathname === '/admin/meter-allocation-prototype' ? '検針配賦の試作' : location.pathname === '/admin/schema-explorer' ? 'スキーマ探索' : location.pathname === '/settings/assets' ? 'アセット設定' : location.pathname === '/settings/tenant-billing' ? 'テナント請求設定' : location.pathname === '/contracts' ? '契約業務フロー' : location.pathname.startsWith('/contract-documents') || location.pathname.endsWith('/document') ? '契約書作成' : location.pathname === '/accounts' ? 'アカウント管理' : location.pathname === '/billing-codes' ? 'テナント請求' : location.pathname === '/procurement' ? '発注・請求・支払管理' : location.pathname === '/financial' ? '収支管理' : location.pathname === '/rent-roll' ? 'レントロール' : location.pathname === '/parking' ? '駐車場台帳' : location.pathname === '/leasing-map' ? 'リーシング図面' : 'ダッシュボード';
