  ...
  const load = async () => {
    ...
    const [codeResult, unitResult, assetResult, settingResult, typeResult] = await Promise.all([
      supabase
        .from("billing_code")
        .select(
          "billing_code_id, tenant_id, issue_code, recipient_name, notes, is_active, is_primary, match_status, flags:billing_code_charge_flag(charge_type, is_enabled), assignments:billing_code_assignment(billing_code_id, lease_contract_unit_id, charge_type, effective_from, effective_to)",
        )
