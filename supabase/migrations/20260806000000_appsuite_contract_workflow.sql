-- AppSuite決裁を起点とする契約業務フロー

alter table public.appsuite_record
  add column if not exists workflow_completed_at timestamptz,
  add column if not exists workflow_completed_by uuid references auth.users (id) on delete set null,
  add column if not exists workflow_processed_at timestamptz,
  add column if not exists workflow_processed_reason varchar(50);

alter table public.lease_contract
  add column if not exists workflow_processed_at timestamptz,
  add column if not exists workflow_processed_reason varchar(50);

-- 既存のレントロール取込契約は、導入時点でフロー処理済みとして履歴を残す。
update public.lease_contract
   set workflow_processed_at = coalesce(workflow_processed_at, now()),
       workflow_processed_reason = coalesce(workflow_processed_reason, 'legacy_rent_roll')
 where source_system = 'rent_roll_xlsx'
   and workflow_processed_at is null;

create index if not exists ix_appsuite_record_contract_workflow_queue
  on public.appsuite_record (approval_status, is_present, workflow_processed_at)
  where is_present and workflow_processed_at is null;
  ...
  if active_match_count = 1 then
    update public.appsuite_record
       set workflow_processed_at = now(),
           workflow_processed_reason = 'active_contract_completed'
...
create or replace function public.trg_reconcile_appsuite_contract_workflow_record()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  perform public.reconcile_appsuite_contract_workflow(new.appsuite_record_id);
  return new;
end;
$$;

create or replace function public.trg_reconcile_appsuite_contract_workflow_contract()
returns trigger
language plpgsql
set search_path = public
as $$
begin
  perform public.reconcile_appsuite_contract_workflow_for_contract(coalesce(new.lease_contract_id, old.lease_contract_id));
  if tg_op = 'DELETE' then return old; end if;
  return new;
end;
$$;

create or replace function public.trg_reconcile_appsuite_contract_workflow_unit()
returns trigger
language plpgsql
set search_path = public
as $$
...
drop trigger if exists trg_reconcile_appsuite_contract_workflow_record on public.appsuite_record;
create trigger trg_reconcile_appsuite_contract_workflow_record
after insert or update of property_name, tenant_name, approval_status, is_present, workflow_completed_at
on public.appsuite_record
for each row execute function public.trg_reconcile_appsuite_contract_workflow_record();

drop trigger if exists trg_reconcile_appsuite_contract_workflow_contract on public.lease_contract;
create trigger trg_reconcile_appsuite_contract_workflow_contract
after insert or update of contract_status, tenant_id
on public.lease_contract
for each row execute function public.trg_reconcile_appsuite_contract_workflow_contract();

drop trigger if exists trg_reconcile_appsuite_contract_workflow_unit on public.lease_contract_unit;
create trigger trg_reconcile_appsuite_contract_workflow_unit
after insert or update or delete
on public.lease_contract_unit
for each row execute function public.trg_reconcile_appsuite_contract_workflow_unit();
...
begin
  ...
  select
    ...
    record.tenant_name,
    coalesce(record.source_updated_at, record.source_created_at),
    record.source_updated_at,
    record.workflow_completed_at,
    record.workflow_completed_by,
    case
...
begin
  if not exists (
    ...
    raise exception 'Contract workflow management permission required';
  end if;
