-- 内部駐車料の合計額を主契約賃料以下に制限しない。
-- 先行期間を外部扱いに分割する公開ラッパーは維持し、その呼び出し先だけを差し替える。

create or replace function public.set_parking_fee_history_base(
...
revoke all on function public.set_parking_fee_history_base(uuid, text, uuid, numeric, date, uuid, uuid, text, text, integer)
  from public, anon;
grant execute on function public.set_parking_fee_history_base(uuid, text, uuid, numeric, date, uuid, uuid, text, text, integer)
  to authenticated;

comment on function public.set_parking_fee_history_base(uuid, text, uuid, numeric, date, uuid, uuid, text, text, integer)
  is '駐車料履歴を適用開始日単位で保存する。内部駐車料合計を主契約賃料で制限しない。';
