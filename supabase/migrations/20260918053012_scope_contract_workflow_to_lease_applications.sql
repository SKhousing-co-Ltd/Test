-- The legacy contract workflow queue predates AppSuite application
-- classification and currently treats every approved record as a contract.
-- Route records using appsuite_application, which is the business-routing
-- source of truth, so procurement records such as SK-SHU never enter this flow.
...
begin
  select record.workflow_completed_at is not null
    ...
    join public.appsuite_application as application on application.app_id = record.app_id
   where record.appsuite_record_id = p_appsuite_record_id
     and application.business_domain = 'lease_contract';
  ...
  select count(distinct contract.lease_contract_id)
    ...
    from public.appsuite_record as record
    join public.appsuite_application as application
      on application.app_id = record.app_id
     and application.business_domain = 'lease_contract'
...
begin
  ...
  select record.appsuite_record_id::uuid,
   ...
   where record.is_present
     and record.approval_status in ('社長決裁済', '完了')
     and record.workflow_processed_at is null
   order by coalesce(record.source_updated_at, record.source_created_at) desc nulls last,
...
begin
  ...
  update public.appsuite_record as record
     set workflow_completed_at = coalesce(record.workflow_completed_at, now()),
         workflow_completed_by = coalesce(record.workflow_completed_by, auth.uid())
    from public.appsuite_application as application
   where record.appsuite_record_id = p_appsuite_record_id
     and application.app_id = record.app_id
     and application.business_domain = 'lease_contract'
     and record.is_present
     and record.approval_status in ('社長決裁済', '完了')
     and record.workflow_processed_at is null;
