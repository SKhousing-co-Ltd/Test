-- Publish a new immutable Word template revision that retains the approved
-- construction/equipment restoration tables. Existing generated output keeps
-- its v1-v3 template references unchanged.
with template_row as (
  ...
  select source.contract_document_template_id,
    ...
    'published',
    'document_generation',
    'templates/ordinary_lease/ordinary_lease_document_generation_v4.docx',
    jsonb_build_object(
      'templateVersion', 'ordinary_lease_docx_v4',
      'planImageKey', 'planImage',
      'restorationNotesKey', 'restorationNotes',
      'restorationRendering', 'approved_static_tables_with_contract_notes'
    ),
  ...
  where not exists (
    ...
    where existing.contract_document_template_id = source.contract_document_template_id
      and existing.template_docx_file_path = 'templates/ordinary_lease/ordinary_lease_document_generation_v4.docx'
  )
...
where document.lease_contract_id = '234fbe6a-4c74-484f-8a33-dfc6178234d9'
  and revision.template_docx_file_path = 'templates/ordinary_lease/ordinary_lease_document_generation_v4.docx';
