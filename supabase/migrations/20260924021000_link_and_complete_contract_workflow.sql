...
begin
  ...
  if not exists (
    select 1 from public.appsuite_record record
    join public.appsuite_application application on application.app_id = record.app_id
    where record.appsuite_record_id = p_appsuite_record_id
      ...
      and record.approval_status in ('遉ｾ髟ｷ豎ｺ陬∵ｸ・, '螳御ｺ・)
  ) then raise exception 'Workflow item is unavailable or already processed'; end if;
  ...
  if coalesce(array_length(p_lease_contract_unit_ids, 1), 0) > 0 and exists (
    ...
    where not exists (
      ...
      where unit.lease_contract_unit_id = selected_unit
        and unit.lease_contract_id = p_lease_contract_id
    )
  ) then raise exception 'Selected contract units must belong to the selected contract'; end if;

  if coalesce(array_length(p_lease_contract_unit_ids, 1), 0) > 0 then
    perform public.confirm_workflow_contract_links(
      p_appsuite_record_id, p_lease_contract_id, p_lease_contract_unit_ids,
      'primary', p_effective_date, 'unit'
    );
  else
    perform public.confirm_workflow_contract_link(
      p_appsuite_record_id, p_lease_contract_id, 'primary', p_effective_date
    );
  end if;
