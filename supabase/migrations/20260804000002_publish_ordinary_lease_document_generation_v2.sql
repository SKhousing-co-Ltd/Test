-- Publish the legal ordinary-lease DOCX template as a new immutable revision.
-- Existing Word/PDF output records stay pinned to the v1 revision.
with template_row as (
  ...
  select source.contract_document_template_id,
    ...
    'published',
    'document_generation',
    'templates/ordinary_lease/ordinary_lease_document_generation_v2.docx',
    '{"templateVersion":"ordinary_lease_docx_v2","planImageKey":"planImage"}'::jsonb,
  ...
  where not exists (
    ...
    where existing.contract_document_template_id = source.contract_document_template_id
      and existing.template_docx_file_path = 'templates/ordinary_lease/ordinary_lease_document_generation_v2.docx'
  )
...
where document.lease_contract_id = '234fbe6a-4c74-484f-8a33-dfc6178234d9'
  and revision.template_docx_file_path = 'templates/ordinary_lease/ordinary_lease_document_generation_v2.docx';
