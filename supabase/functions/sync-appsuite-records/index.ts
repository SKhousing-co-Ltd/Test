...
async function syncApplication(appId: string) {
  ...
  try {
    const { error: upsertError } = await admin.from('appsuite_record').upsert(enrichedRecords.map((record) => ({ ...record, is_present: true, last_seen_at: now })), { onConflict: 'app_id,data_id' });
